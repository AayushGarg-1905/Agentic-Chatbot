...
if "user" not in st.session_state:
    # local_user = locals.getItem('agentic-chatbot-user')
    # print("Local user: ***************************** ",local_user)
    # if(local_user):
    #     st.session_state['user'] = json.loads(local_user)
    # else:   
...
if st.session_state["user"] is None:
    ...
    if st.button("Login"):
        if username and password and '_' not in password:
            st.session_state["user"] = {"username":username,"password":password} # todo: store password as hash+salt
            # store in local storage
            # locals.setItem('agentic-chatbot-user',json.dumps(st.session_state["user"]))
            st.rerun()
        else:
